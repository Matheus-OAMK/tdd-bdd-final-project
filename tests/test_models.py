...
class TestProductModel(unittest.TestCase):
    ...
    def test_create_a_product(self):
        ...
        copy = Product().deserialize(data)
        self.assertEqual(copy.name, product.name)
        self.assertEqual(copy.description, product.description)
        self.assertEqual(copy.price, Decimal(str(product.price)))
        self.assertEqual(copy.available, product.available)
    ...
    def test_list_all_products(self):
        ...
        self.assertGreaterEqual(Product.find_by_price(price).count(), 1)
        self.assertGreaterEqual(
            Product.find_by_price(f' "{price}" ').count(), 1)
    ...
    def test_find_by_category(self):
        ...
        with self.assertRaises(DataValidationError):
            Product().deserialize(missing_key)
        with self.assertRaises(DataValidationError):
            Product().deserialize(invalid_category)
        with self.assertRaises(DataValidationError):
